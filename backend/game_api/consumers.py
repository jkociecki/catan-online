...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        try:
            ...
            if message_type == 'get_game_state':
                ...
                # Process game actions with new system
                action = data.get('action')
                print(f"Received game action: {action} with data: {data}")
                ...
                if is_setup_phase:
                    ...
                    if action == 'build_settlement':
                        result = await self.process_build_settlement_new(game_state, player, data, is_setup=True)
                        if result:
                            await self.channel_layer.group_send(
                                self.room_group_name,
                                {
                        ...
                        result = await self.process_build_road_new(game_state, player, data, is_setup=True)
                        if result:
                            await self.channel_layer.group_send(
                                self.room_group_name,
                                {
                    ...
                    # Main game phase actions
                    if action == 'roll_dice':
                        # Handle dice roll
                        pass
                    elif action == 'build_settlement':
                        result = await self.process_build_settlement_new(game_state, player, data, is_setup=False)
                        # Handle result...
                    elif action == 'build_city':
                        result = await self.process_build_city_new(game_state, player, data)
                        # Handle result...
                    elif action == 'build_road':
                        result = await self.process_build_road_new(game_state, player, data, is_setup=False)
                        # Handle result...
    ...
    # ========== NOWE METODY PRZETWARZANIA Z INDEKSAMI ==========

    @database_sync_to_async
    def process_build_settlement_new(self, game_state, player, data, is_setup=False):
        """Przetwórz budowanie osady używając nowego systemu indeksów"""
        try:
            print(f"Processing build settlement for player {player.id}")

            # Pobierz dane z różnych formatów
            tile_id_str = data.get('tileId')
            corner_index = data.get('cornerIndex')
            coords = data.get('coords')

            # Wariant 1: tileId + cornerIndex (preferowany)
            if tile_id_str is not None and corner_index is not None:
                print(f"Using tileId={tile_id_str}, cornerIndex={corner_index}")
                
                # Konwertuj string tile_id na współrzędne
                try:
                    tile_coords = tuple(map(int, tile_id_str.split(',')))
                    print(f"Converted tileId to coordinates: {tile_coords}")
                    
                    # Użyj nowej metody
                    result = game_state.place_settlement_by_tile_coords(
                        player, tile_coords, corner_index, is_setup
                    )
                    return result
                    
                except Exception as e:
                    print(f"Error converting tileId: {str(e)}")
                    return False

            # Wariant 2: coords (stary format)
            elif coords:
                print(f"Using legacy coords format: {coords}")
                
                # Próbuj użyć pierwszych koordynatów jako tile_coords
                if isinstance(coords, list) and len(coords) > 0:
                    first_coord = coords[0]
                    if isinstance(first_coord, list) and len(first_coord) == 3:
                        tile_coords = tuple(first_coord)
                        
                        # Użyj domyślnego corner_index = 0
                        result = game_state.place_settlement_by_tile_coords(
                            player, tile_coords, 0, is_setup
                        )
                        return result

            print("No valid coordinates provided for settlement")
            return False

        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"Error in process_build_settlement_new: {str(e)}")
            return False

    @database_sync_to_async
    def process_build_road_new(self, game_state, player, data, is_setup=False):
        """Przetwórz budowanie drogi używając nowego systemu indeksów"""
        try:
            print(f"Processing build road for player {player.id}")
            ...
            # Wariant 1: tileId + edgeIndex (preferowany)
            if tile_id_str is not None and edge_index is not None:
                print(f"Using tileId={tile_id_str}, edgeIndex={edge_index}")
                ...
                try:
                    ...
                    result = game_state.place_road_by_tile_coords(
                        player, tile_coords, edge_index, is_setup
                    )
                    return result
            ...
            # Wariant 2: coords (stary format)
                ...
                if isinstance(coords, list) and len(coords) > 0:
                    ...
                    if isinstance(first_coord, list) and len(first_coord) == 3:
                        ...
                        # Użyj domyślnego edge_index = 0
                        result = game_state.place_road_by_tile_coords(
                            player, tile_coords, 0, is_setup
                        )
                        return result

            print("No valid coordinates provided for road")
            return False

        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"Error in process_build_road_new: {str(e)}")
            return False

    @database_sync_to_async
    def process_build_city_new(self, game_state, player, data):
        """Przetwórz budowanie miasta używając nowego systemu indeksów"""
        try:
            print(f"Processing build city for player {player.id}")
            ...
            if tile_id_str is not None and corner_index is not None:
                ...
                try:
                    ...
                    # Znajdź tile_id i vertex_id
                    result = game_state.game_board.get_tile_by_coords(tile_coords)
                    if not result:
                        return False
                    
                    tile_id, _ = result
                    vertex_id = game_state.game_board.find_vertex_by_tile_and_corner(tile_id, corner_index)
                    if vertex_id is None:
                        return False
                    
                    # Użyj nowej metody
                    result = game_state.place_city_by_id(player, vertex_id)
                    return result
            ...
            print("No valid coordinates provided for city")
            return False

        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"Error in process_build_city_new: {str(e)}")
            return False
    ...
    async def process_end_turn(self, game_state):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            print(f"Error during end turn: {str(e)}")
            return False

    def serialize_game_state(self, game_state):
        """Serializuj stan gry używając nowego systemu"""
        try:
            print("Serializing game state with new system")
            
            # Użyj nowej metody serializacji z GameBoard
            ...
            for player in game_state.players:
                player_data = {
                    'id': player.id,
                    'color': player.color.value if hasattr(player.color, 'value') else str(player.color),
                    'resources': player.player_resources.serialize() if hasattr(player.player_resources, 'serialize') else (
                        player.player_resources.resources if hasattr(player.player_resources, 'resources') else {}
                    ),
                    'victory_points': player.victory_points if hasattr(player, 'victory_points') else 0,
                    'settlements_left': player.settlements_left if hasattr(player, 'settlements_left') else 5,
